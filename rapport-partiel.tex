...
Les nombres de Stirling croissent tr\`es rapidement, ce qui n\'ecessite
l'utilisation de grands entiers. TypeScript offre un support natif pour cela
via le type \texttt{bigint}. Un cache LRU permet de m\'emoïser les appels
r\'ecursifs et d'\'eviter les recalculs. Ces d\'etails ont \'et\'e omis des
exemples de code ci-dessous par souci de clart\'e.

\begin{minted}{typescript}
function stirling_numbers(n: number, k: number): bigint {
  if (n === 0 && k === 0) return 1n;
  if (n === 0 || k === 0) return 0n;
  const res = stirling_numbers(n - 1, k - 1) + BigInt(k) * stirling_numbers(n - 1, k);
  return res;
}
function unrank_stirling(n: number, k: number, r: number): number[][] {
  if (n === 0 || k === 0) return [];
...
L'algorithme suit la m\^eme logique que pour les partitions de Stirling
classiques, \`a ceci pr\`es que l'ordre des blocs est significatif.

\begin{minted}{typescript}
function ordered_stirling_numbers(n: number, k: number): bigint {
  if (k === 1 || k === n) return factorial(k);
  const result = BigInt(k) * (ordered_stirling_numbers(n - 1, k - 1) + ordered_stirling_numbers(n - 1, k));
  return result;
}
function unrank_perm(n: number, r: number): number[] {
...
function unrank_ordered_stirling(n: number, k: number, r: number): number[][] {
...
\begin{minted}{typescript}
function lah_numbers(n: number, k: number): bigint {
  if (n === 0 && k === 0) return 1n;
  if (n === 0 || k === 0) return 0n;
  if (n === k) return 1n;
...
function unrank_lah(n: number, k: number, r: number): number[][] {
  if (n === 0) return [];
...
\section{Partitions de Lah Ordonn\'ees}

\subsection{Formule de comptage}

Le nombre de partitions de Lah ordonn\'ees de $\{1, \ldots, n\}$ en $k$ blocs est
not\'e $\left\lceil n \atop k \right\rceil$ et satisfait la r\'ecurrence suivante :

\[
  \left\lceil n \atop k \right\rceil =
  \begin{cases}
    n! & \text{si } k = 1 \text{ ou } k = n \\[6pt]
    k \cdot \left\lceil n-1 \atop k-1 \right\rceil + (n - 1 + k) \cdot
    \left\lceil n-1 \atop k \right\rceil & \text{sinon}
  \end{cases}
\]

\subsection{Interpr\'etation combinatoire}

Une partition de Lah ordonn\'ee est la structure la plus riche parmi les quatre
partitions \'etudi\'ees : l'ordre des \'el\'ements au sein de chaque bloc compte,
et l'ordre des blocs entre eux compte \'egalement. Elle combine ainsi les
caract\'eristiques des partitions de Stirling ordonn\'ees (blocs ordonn\'es) et
des partitions de Lah (s\'equences internes).

La r\'ecurrence se comprend comme suit : pour construire une $k$-partition de
$\{1, \ldots, n\}$, soit on ajoute $n$ comme nouveau bloc singleton dans l'une
des $k$ positions possibles parmi les blocs d'une $(k-1)$-partition de
$\{1, \ldots, n-1\}$, soit on ins\`ere $n$ dans l'un des blocs existants d'une
$k$-partition de $\{1, \ldots, n-1\}$, en $n - 1 + k$ positions possibles.

Aux cas de base, si $k = 1$ tous les \'el\'ements forment une seule s\'equence,
soit $n!$ arrangements. Si $k = n$ chaque \'el\'ement est seul dans son bloc et
toute permutation des blocs donne une partition diff\'erente, soit \'egalement $n!$
partitions.

\subsection{Impl\'ementation}

\begin{minted}{typescript}
function ordered_lah_numbers(n: number, k: number): bigint {
...
function unrank_ordered_lah(n: number, k: number, r: number): number[][] {
  if (n === 0) return [];
...
\begin{minted}{typescript}
function int_partitions(n: number, k: number): bigint {
  if (n < 1) return 0n;
  if (k === 1 || n === k) return 1n;
  const result = int_partitions(n - 1, k - 1) + int_partitions(n - k, k);
  return result;
}
function unrank_int_partitions(n: number, k: number, r: number): number[] {
  if (n < 1) return [];
  if (k === 1) return [n];
...
Le code source est versionn\'e avec Git et disponible \`a l'adresse suivante:
\url{https://github.com/qyrhx/Unranking-Algos}. L'utilisation de Git facilite le suivi des modifications et
la collaboration entre les membres de l'\'equipe.
...
complexes. La biblioth\`eque \texttt{lru-cache} est \'egalement utilis\'ee pour la
m\'emo\"isation des fonctions de comptage, dont les valeurs sont r\'eutilis\'ees
intensivement lors des appels r\'ecursifs.
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.5\textwidth]{images/webapp_main.png}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.5\textwidth]{images/webapp_unrank.png}
...
\begin{figure}[h]
  \centering
  \includegraphics[width=0.5\textwidth]{images/webapp_listall.png}
